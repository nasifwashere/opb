...
function createShopEmbed(category, items) {
  ...
  const embed = new EmbedBuilder()
    .setTitle(`🛍️ Shop - ${categoryNames[category] || 'All Items'}`)
...
async function execute(message, args) {
  ...
  embed.addFields({ name: 'Your Beli', value: `💰 ${user.beli || 0}`, inline: false });
  ...
  collector.on('collect', async interaction => {
    ...
    if (interaction.customId.startsWith('shop_')) {
      ...
      newEmbed.addFields({ name: 'Your Beli', value: `💰 ${updatedUser.beli || 0}`, inline: false });
