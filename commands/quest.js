  ...
  collector.on('collect', async interaction => {
    ...
    if (interaction.customId.startsWith('quest_')) {
      ...
      if (['daily', 'weekly', 'story'].includes(action)) {
        ...
        const newEmbed = createQuestEmbed(currentType, questsByType[currentType], user);
        ...
        if (claimedRewards.length > 0) {
          ...
          const refreshedQuests = await getAvailableQuests(user);
          ...
          const newEmbed = createQuestEmbed(currentType, refreshedQuestsByType[currentType], user);
          const newComponents = createQuestButtons(currentType);
          await questMessage.edit({ embeds: [newEmbed], components: newComponents });
        } else {
